...
### Демо-режим (без ключей Ozon API)

Попробуйте все 13 инструментов с реалистичными данными — аккаунт продавца Ozon, Redis и PostgreSQL не нужны:

```bash
DEMO_MODE=true uv run python -m ozon_mcp_server
```

В демо-режиме: 5 товаров (AirPods, Dyson, JBL, Nike, Samsonite), 4 заказа из 4 городов, 3 склада, аналитика продаж (2,1 млн руб. / 150 заказов) и финансовые отчёты.
